  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white mt-1">✨ 오늘의 일지 자동 작성</h1>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500">git 커밋 + 미커밋 진행 중인 변경까지 모아 AI 가 개발일지 한 편을 써줍니다.</p>
      </div>

      <div className="space-y-5">
        <div>
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 block mb-1">날짜</label>
        ...
        <div>
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 block mb-1">
            오늘 한 일 메모 <span className="text-slate-400 dark:text-slate-500 font-normal">(선택 — 키워드만 적어도 OK)</span>
          </label>
          <textarea value={note} onChange={e => setNote(e.target.value)}
            placeholder="예) Cloudflare 터널 설정, MacroDroid SMS 자동 입력 422 통과, blog 디자인 Apple 스타일 리뉴얼"
            rows={4}
            className="w-full border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm bg-white dark:bg-slate-900 leading-relaxed focus:outline-none focus:border-slate-900 dark:focus:border-slate-100" />
        </div>

        <div>
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 block mb-1">
            추가 git 경로 <span className="text-slate-400 dark:text-slate-500 font-normal">(한 줄에 하나 — 이 블로그 외 다른 프로젝트도 포함하려면)</span>
          </label>
          <textarea value={repos} onChange={e => setRepos(e.target.value)}
            placeholder={"예)\nC:\\my-calendar-app\nD:\\projects\\macrodroid-scripts"}
            rows={3}
            className="w-full border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm bg-white dark:bg-slate-900 font-mono leading-relaxed focus:outline-none focus:border-slate-900 dark:focus:border-slate-100" />
        </div>

        <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800/50 transition-colors">
          <input type="checkbox" checked={includeUncommitted}
            onChange={e => setIncludeUncommitted(e.target.checked)}
            className="mt-0.5 w-4 h-4 accent-indigo-600" />
          <div>
            <div className="text-sm font-semibold text-slate-800 dark:text-slate-100">아직 커밋 안 한 작업도 포함</div>
          ...
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 px-4 py-3">
            ...
            {progress?.activities && progress.activities.length > 0 && (
              <div className="mt-3 pt-3 border-t border-indigo-100">
                <p className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1">수집된 활동</p>
                <ul className="text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500 space-y-1">
                  {progress.activities.map(a => {
                    ...
                    return (
                      <li key={a.repo} className="font-mono">
                        • <b className="text-slate-700 dark:text-slate-200">{a.repo}</b>
        ...
        {/* 결과 미리보기 */}
        {result && (
          <div className="border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900 p-6 mt-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xs font-semibold text-indigo-600 uppercase tracking-wider">생성된 일지</div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white mt-1">{result.title}</h2>
              </div>
            ...
            <pre className="text-sm text-slate-700 dark:text-slate-200 whitespace-pre-wrap font-mono leading-relaxed max-h-[36rem] overflow-y-auto bg-slate-50 dark:bg-slate-900 rounded-lg p-4">{result.body}</pre>
          </div>
        )}
      </div>
    </div>
